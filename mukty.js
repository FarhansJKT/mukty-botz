...
//antilinkgrup
		        ...
		        setTimeout( () => {
			        mukty.groupRemove(from, [kic]).catch((e)=>{reply(`*NICO HARUS JADI ADMIN├в┬ЭтАФ*`)})
		        }, 3000)
		        setTimeout( () => {
			        mukty.updatePresence(from, Presence.composing)
			        reply("Hedsot....")
...
case 'speed'://UPDATE FIX BUG
				...
				if (args.length < 1) return mukty.sendMessage(from, 'Kode bahasanya mana kak? contoh : ${prefix}Nakano Miku', text, {quoted: mek})
                 ...
                 if (!isRegistered) return reply(ind.noregis())
					...
					setTimeout( () => {
					mukty.sendMessage(from, '*➸ Jawaban :* '+anu.result.jawaban, text, {quoted: mek}) // ur cods
					}, 30000) // 1000 = 1s,
					setTimeout( () => {
					mukty.sendMessage(from, '_10 Detik lagi…_', text) // ur cods
					}, 20000) // 1000 = 1s,
					...
					setTimeout( () => {
					mukty.sendMessage(from, '*➸ Jawaban :* '+anu.result.name, text, {quoted: mek}) // ur cods
					}, 30000) // 1000 = 1s,
					setTimeout( () => {
					mukty.sendMessage(from, '_10 Detik lagi…_', text) // ur cods
					}, 20000) // 1000 = 1s,
...
if (isBanned) return reply(mess.only.benned)
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					reply(ind.wait())
					anu = await fetchJson(`https://fdciabdul.tech/api/pinterest?keyword=Kpop`, {method: 'get'})
...
case 'neko'://UPDATE MUKTY
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
			       ...
			       if (isLimit(sender)) return reply(ind.limitend(pusname))
					reply(ind.wait())	
					anu = await fetchJson(`https://fdciabdul.tech/api/pinterest?keyword=anime+loli`, {method: 'get'})
            ...
            if (!isRegistered) return reply(ind.noregis())
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					mukty.sendMessage(from, pok, image, { quoted: mek, caption: `*𝐏𝐈𝐍𝐓𝐄𝐑𝐄𝐒𝐓*`})
					await limitAdd(sender)
					break 
			...
			if (!isRegistered) return reply(ind.noregis())
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					reply(ind.wait())
					anu = await fetchJson(`https://fdciabdul.tech/api/pinterest?keyword=Kaneki-ken`, {method: 'get'})
					sasu = JSON.parse(JSON.stringify(anu));
					ke =  sasu[Math.floor(Math.random() * sasu.length)];
					nye = await getBuffer(ke)
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				reply(ind.wait())
					anu = await fetchJson(`https://fdciabdul.tech/api/pinterest?keyword=anime+touka`, {method: 'get'})
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					reply(ind.wait())
					anu = await fetchJson(`https://fdciabdul.tech/api/pinterest?keyword=anime+rize`, {method: 'get'})
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					reply(ind.wait())
					anu = await fetchJson(`https://fdciabdul.tech/api/pinterest?keyword=anime+akira`, {method: 'get'})
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					reply(ind.wait())
					anu = await fetchJson(`https://fdciabdul.tech/api/pinterest?keyword=anime+itori`, {method: 'get'})
		...
		case 'miku'://UPDATE MUKTY
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					reply(ind.wait())
					anu = await fetchJson(`https://fdciabdul.tech/api/pinterest?keyword=Nakano Miku`, {method: 'get'})
...
case 'rem'://UPDATE MUKTY
			...
			if (isLimit(sender)) return reply(ind.limitend(pusname))
					reply(ind.wait())	
					anu = await fetchJson(`https://fdciabdul.tech/api/pinterest?keyword=rem`, {method: 'get'})
					sasu = JSON.parse(JSON.stringify(anu));
					ke =  sasu[Math.floor(Math.random() * sasu.length)];
					nye = await getBuffer(ke)
			...
			if (isLimit(sender)) return reply(ind.limitend(pusname))
					reply(ind.wait())	
					anu = await fetchJson(`https://fdciabdul.tech/api/pinterest?keyword=sagiri`, {method: 'get'})
					sasu = JSON.parse(JSON.stringify(anu));
					ke =  sasu[Math.floor(Math.random() * sasu.length)];
					nye = await getBuffer(ke)
   ...
   case 'yotsuba'://UPDATE MUKTY
			...
			if (isLimit(sender)) return reply(ind.limitend(pusname))
					reply(ind.wait())	
					anu = await fetchJson(`https://fdciabdul.tech/api/pinterest?keyword=yotsuba`, {method: 'get'})
					sasu = JSON.parse(JSON.stringify(anu));
					ke =  sasu[Math.floor(Math.random() * sasu.length)];
					nye = await getBuffer(ke)
...
case 'animesad'://UPDATE MUKTY
			...
			if (isLimit(sender)) return reply(ind.limitend(pusname))
					reply(ind.wait())	
					anu = await fetchJson(`https://fdciabdul.tech/api/pinterest?keyword=Anime sad`, {method: 'get'})
					sasu = JSON.parse(JSON.stringify(anu));
					ke =  sasu[Math.floor(Math.random() * sasu.length)];
					nye = await getBuffer(ke)
...
case 'rikka'://UPDATE MUKTY
			...
			if (isLimit(sender)) return reply(ind.limitend(pusname))
					reply(ind.wait())	
					anu = await fetchJson(`https://fdciabdul.tech/api/pinterest?keyword=rikka`, {method: 'get'})
					sasu = JSON.parse(JSON.stringify(anu));
					ke =  sasu[Math.floor(Math.random() * sasu.length)];
					nye = await getBuffer(ke)
					mukty.sendMessage(from, nye, image, { caption: 'Kawaii kan', quoted: mek })
            ...
            if (!isRegistered) return reply(ind.noregis())
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					reply(ind.wait())
					anu = await fetchJson(`https://fdciabdul.tech/api/pinterest?keyword=Ohto Ai`, {method: 'get'})
					naru = JSON.parse(JSON.stringify(anu));
					to =  naru[Math.floor(Math.random() * naru.length)];
					nye = await getBuffer(to)
...
case 'antidelete':
           ...
           case 'setname':
                ...
                mukty.sendMessage(from, '⟪ SUKSES ⟫ Mengubah Nama Grup', text, {quoted: mek})
					break
                case 'setdesc':
                //UPDATE FHANS                
                if (!isGroup) return reply(ind.groupo())
                ...
                mukty.sendMessage(from, '⟪ SUKSES ⟫ Mengubah Desk Grup', text, {quoted: mek})
					break
           case 'demote':
                //UPDATE FHANS           
					if (!isGroup) return reply(ind.groupo())
...
case 'notoxic':
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							mukty.sendMessage(_.jid, buff, image, {caption: `*「 REM BOT 」*\n\n${body.slice(4)}`})
						}
						reply('')
					} else {
						for (let _ of anu) {
...
if (budy.includes(`@6281393668101`)) {
                  reply(`Jangan Tag FHANS Kak, Dia Lagi Sibuk ✨`)
                  }
